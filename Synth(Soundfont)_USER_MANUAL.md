...
이 문서는 현재 웹앱을 실제로 조작하는 방법을 기준으로 작성한 상세 매뉴얼입니다. 화면의 버튼과 노브는 실제 기능이 연결된 컨트롤 중심으로 구성되어 있습니다.
...
현재 화면에서는 실제 기능이 연결되지 않은 장식용 버튼과 노브를 제거했습니다. 일부 페이지 상단의 섹션 라벨은 현재 위치를 알려주는 안내 텍스트이며, 클릭 조작용 버튼으로 사용하지 않습니다.
